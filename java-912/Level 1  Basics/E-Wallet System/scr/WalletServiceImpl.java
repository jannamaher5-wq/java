 ...
 * Implements WalletService interface containing core business logic,
 * financial validation, daily limits, and administrative controls.
 */
public class WalletServiceImpl implements WalletService {
    ...
    // Constructor initializing account store with a default Admin account
    public WalletServiceImpl() {
        this.accounts = new ArrayList<>();
        this.loggedInUser = null;

        String adminHash = SecurityUtils.hashPassword("IAM123");
        Account admin = new Account("IAM", adminHash, 30, "01000000000", true);
        accounts.add(admin);
    }
    ...
    public boolean register(String username, String rawPassword, int age, String phone) throws Exception {
        ...
        Account newAccount = new Account(username, hashedPassword, age, phone, false);
    ...
    public boolean login(String username, String rawPassword) throws Exception {
        ...
        if (!account.isActive()) {
            throw new Exception("Access denied! Account is currently inactive/frozen. Contact Admin.");
        }
    ...
    public void withdraw(double amount) throws Exception {
        ...
        if (amount > loggedInUser.getBalance()) {
            throw new Exception("Insufficient funds! Transaction exceeds current available balance.");
        }
        loggedInUser.setBalance(loggedInUser.getBalance() - amount);
    }

    @Override
    public void transfer(String destinationUsername, double amount) throws Exception {
        checkLoggedIn();
    ...
    public void changePassword(String oldRawPassword, String newRawPassword) throws Exception {
        ...
        String oldHash = SecurityUtils.hashPassword(oldRawPassword);
        ...
        String newHash = SecurityUtils.hashPassword(newRawPassword);
